...
const fixture=path.join(root,'candidate/biomes-smoke-'+crypto.randomUUID()),server=path.join(fixture,'server');
...
(async()=>{
 ...
 run('javac',['--release','8','-encoding','UTF-8','-cp',api+';'+jar,'-d',classes,path.join(root,'tests/java/chat/jaspr/biomes/BiomeProbe.java')]);
 write('classes/plugin.yml','name: BiomeProbe\nmain: chat.jaspr.biomes.BiomeProbe\nversion: 1\ndepend: [JasprHorrorBiomes]\ncommands:\n  biomessmoke:\n    description: Isolated biome fixture\n');
 run('jar',['--create','--file',path.join(server,'plugins/BiomeProbe.jar'),'-C',classes,'.']);
 ...
 write('server/plugins/EaglercraftXServer/settings.yml','server_name: Biome fixture\nserver_uuid: "'+crypto.randomUUID()+'"\nprotocols:\n  max_minecraft_protocol: 340\n  eaglerxrewind_allowed: true\n  protocol_v3_allowed: true\n  protocol_v4_allowed: true\nvoice_service:\n  enable_voice_service: false\nupdate_checker:\n  enable_update_checker: false\nupdate_service:\n  enable_update_system: false\n  download_latest_certs: false\n');
 const html=`<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>JasperCraft biome fixture</title><style>html,body,#game_frame{margin:0;width:100%;height:100%;overflow:hidden;background:black}</style><script src="/classes.js"></script><script src="/jaspr-profile.js"></script></head><body><div id="game_frame"></div><script>JasprProfile.prepare('BiomeProbe',localStorage).then(function(){window.eaglercraftXOpts={container:'game_frame',assetsURI:'/assets.epk',localStorageNamespace:'_eaglercraft_1122_tailscale_ui2',localStorageLoaded:function(k){return localStorage.getItem(k.startsWith('_eaglercraft_')?k:'_eaglercraft_1122_tailscale_ui2.'+k);},worldsDB:'biome_preview',resourcePacksDB:'biome_packs',joinServer:'ws://127.0.0.1:${socket}/',servers:[],relays:[],crashOnUncaughtExceptions:true};main();});</script></body></html>`;
 const web=http.createServer((req,res)=>{if(req.url==='/'){res.setHeader('Content-Type','text/html; charset=utf-8');res.end(html);return;}const file={'/classes.js':'candidate/horror-biomes/classes.js','/assets.epk':'site/assets.epk','/jaspr-profile.js':'site/jaspr-profile.js'}[req.url.split('?')[0]];if(!file){res.writeHead(404);res.end();return;}res.setHeader('Content-Type',file.endsWith('.js')?'application/javascript; charset=utf-8':'application/octet-stream');res.setHeader('Cache-Control','no-store');fs.createReadStream(path.join(root,file)).pipe(res);});web.listen(webPort,'127.0.0.1');
 ...
 let started=false,tail='';function out(data){log.write(data);tail=(tail+data).slice(-16000);if(/BIOME_|HORROR_|Done \(|ERROR|SEVERE/.test(String(data)))process.stdout.write(data);if(!started&&tail.includes('Done (')){started=true;child.stdin.write('biomessmoke\n');}}
 ...
 write('fixture.json',JSON.stringify({fixture,socket,webPort,pid:child.pid},null,2));console.log('Biome preview URL: http://127.0.0.1:'+webPort+'/');console.log('Fixture: '+fixture);console.log('Commands: biomessmoke, biomessmoke show 0..61, stop');
})();
