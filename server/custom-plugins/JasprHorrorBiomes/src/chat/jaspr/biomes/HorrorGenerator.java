...
public final class HorrorGenerator extends ChunkGenerator {
    ...
    private void sanctuary(ChunkData d,int[][] ground){
        ...
        for(int x=3;x<=12;x++)for(int z=3;z<=12;z++){
            ...
            // Audit: clear up to the local ground too, so a hillside never leaves terrain hanging over the court.
            for(int y=base+1;y<=Math.max(base+6,g);y++)block(d,x,y,z,0,0);
        }
        ...
        sanctuaryDressing(d,ground,base);
    }
    /** Audit: approach, doorway, coping and court lights for the portal sanctuary (same stone-brick palette). */
    private static void sanctuaryDressing(ChunkData d,int[][] ground,int base){
        // Chiselled jambs and a lintel make the north gap a 2x2 doorway; coping slabs and chiselled caps finish the wall.
        for(int y=1;y<=4;y++){block(d,6,base+y,3,98,3);block(d,9,base+y,3,98,3);}
        block(d,7,base+3,3,98,3);block(d,8,base+3,3,98,3);
        for(int x=3;x<=12;x++)for(int z=3;z<=12;z++)if((x==3||x==12||z==3||z==12)&&!(z==3&&(x==6||x==9)))
            block(d,x,base+4,z,(x==3||x==12)&&(z==3||z==12)?98:44,(x==3||x==12)&&(z==3||z==12)?3:5);
        // Wall torches on the inner faces keep the court above spawn light where players set the eyes.
        block(d,4,base+2,8,50,1);block(d,11,base+2,7,50,2);block(d,7,base+2,11,50,4);block(d,5,base+2,4,50,3);block(d,10,base+2,4,50,3);
        // Approach, z 2..0 in front of the doorway: stone-brick steps down to lower ground or water (base stays clamped
        // at 65, so a shore site gets three steps), a cut path with retaining cheeks into higher ground.
        for(int i=1;i<=3;i++){
            int z=3-i;
            for(int x=6;x<=9;x++){
                int g=ground[x][z];boolean path=x==7||x==8;
                if(g>base){ // uphill: open a walkway at floor level, earth held back by the cheeks
                    if(path){block(d,x,base,z,98,2);for(int y=base+1;y<=g;y++)block(d,x,y,z,0,0);}
                    else for(int y=base+1;y<=Math.min(g,base+4);y++)block(d,x,y,z,98,2);
                }else if(base-i>g){ // downhill or water: a step on a footing, cheeks one lower beside it
                    int top=path?base-i:base-i-1;
                    for(int y=g-1;y<top;y++)block(d,x,y,z,98,2);
                    if(path)block(d,x,top,z,109,2);else block(d,x,top,z,98,2);
                    if(path)for(int y=top+1;y<=top+3;y++)block(d,x,y,z,0,0);
                }
            }
        }
    }
