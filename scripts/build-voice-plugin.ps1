...
$pluginRoot = Join-Path $serverRoot 'custom-plugins\JasprVoiceChat'
...
$outputJar = Join-Path $candidateRoot 'JasprVoiceChat.jar'
...
$compileClasspath = @('cache\patched_1.12.2.jar') | ForEach-Object { Join-Path $serverRoot $_ }
...
Write-Output "Install it with scripts\install-voice-chat.ps1"
