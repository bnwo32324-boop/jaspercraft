...
$pluginRoot = Join-Path $serverRoot 'custom-plugins\JasprInvasions'
...
$candidateRoot = Join-Path $projectRoot 'candidate\invasions'
$classes = Join-Path $candidateRoot ('classes-' + [Guid]::NewGuid().ToString('N'))
$outputJar = Join-Path $candidateRoot 'JasprInvasions.jar'
...
& $javac --release 8 -encoding UTF-8 -cp (Join-Path $serverRoot 'cache\patched_1.12.2.jar') -d $classes $sourceFiles
