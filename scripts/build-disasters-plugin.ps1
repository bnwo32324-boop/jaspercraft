...
$pluginRoot = Join-Path $serverRoot 'custom-plugins\JasprDisasters'
...
$candidateRoot = Join-Path $projectRoot 'candidate\disasters'
$classes = Join-Path $candidateRoot ('classes-' + [Guid]::NewGuid().ToString('N'))
$outputJar = Join-Path $candidateRoot 'JasprDisasters.jar'
