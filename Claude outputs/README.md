...
Ten rocks fall out of the sky around the target over about fifteen seconds. Each one detonates on
impact with a creeper-grade blast, scattering magma blocks and fire around the crater. A meteor that
...
terrain intact. `state.yml` holds the next scheduled time for each kind so a restart does not reset
the cycle.
